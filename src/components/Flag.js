...
//use params
  ...
  return (
    <div key={countryData?.id} role='button' onClick={() => {navigate(`/details/${countryData?.name.common}`)}} className= 'mt-8 w-[100%] lg:grid lg:grid-cols-4 lg:w-[100%] items-center lg:mx-auto sm:-ml-[95%] sm:grid sm:grid-cols-2 sm:w-[200%] sm:mx-auto lg:-ml-24 md:grid md:grid-cols-2 md:w-[200%] md:mx-auto md:-ml-[110%]'>
        <p>{id}</p>
        <div id={countryData?.name.common} role='button' onClick={Handlechange}  className= {`${darkTheme ? "bg-[#2b3944] text-white" : "bg-[#f2f2f2]"} bg-white w-[80%] lg:w-[350%] h-96 mx-auto mt-8 rounded-sm font-sans shadow-lg md:ml-20 lg:ml-10`}>
